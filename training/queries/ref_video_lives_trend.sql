-- Provenance: _private/daily_reporting/reference/queries_final/04_video_lives_trend.sql
-- Purpose:    Frozen reference form (hard-coded date range). Useful as a template; for parameterized form see the matching `video_lives_trend.sql`.
...
-- Video/Lives Trend
-- 30-day query. Daily GMV for Video and Livestream content types.
-- Same join approach as Content Type Split.

SELECT
    DATE(t.created_time)                                                   AS d,
    ROUND(COALESCE(SUM(
...
WHERE t.created_time >= '2026-03-17 00:00:00'
