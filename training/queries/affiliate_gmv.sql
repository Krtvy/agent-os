-- Provenance: _private/excel_automation/automation/reports/affiliate_gmv.py · QUERY
-- Purpose:    Per-day, per-creator, per-product, per-content-type GMV for TikTok affiliate orders. Splits traffic into 'shop_ads' vs 'organic'.
...
SELECT
    ...
    a.content_id,
    NULL::date                                                              AS post_date,
    CASE
        WHEN COALESCE(a.actual_shop_ads_commission_payment, a.est_shop_ads_commission_payment, 0) > 0
        THEN 'shop_ads'
        ELSE 'organic'
    END                                                                     AS traffic_type,
