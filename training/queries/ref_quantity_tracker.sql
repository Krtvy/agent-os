-- Provenance: _private/daily_reporting/reference/queries_final/02_quantity_tracker.sql
-- Purpose:    Frozen reference form (hard-coded date range). Useful as a template; for parameterized form see the matching `quantity_tracker.sql`.
...
-- Quantity Tracker
-- 3-day query. Replace date range for target 3 days.
-- Units = quantity * unit_multiplier. Excludes all-zero variants via INNER JOIN.
-- Filters: cancelled orders excluded, zero-price excluded, active products/listings.

SELECT
    rp.rootlabs_common_name                                              AS product,
    rp.sku_name                                                          AS variation,
    COALESCE(SUM(t.quantity * COALESCE(rp.unit_multiplier, 1))
        FILTER (WHERE DATE(t.created_time) = DATE '2026-04-13'), 0)      AS apr_13,
    COALESCE(SUM(t.quantity * COALESCE(rp.unit_multiplier, 1))
        FILTER (WHERE DATE(t.created_time) = DATE '2026-04-14'), 0)      AS apr_14,
    COALESCE(SUM(t.quantity * COALESCE(rp.unit_multiplier, 1))
        FILTER (WHERE DATE(t.created_time) = DATE '2026-04-15'), 0)      AS apr_15
